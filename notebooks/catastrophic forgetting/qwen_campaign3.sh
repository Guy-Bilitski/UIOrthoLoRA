#!/usr/bin/env bash
# Qwen2.5-7B half of the intruder table -- v3 (2026-09-14 21:40 UTC), CONCURRENT schedule, RESTARTABLE.
#
# v3 = v2 + (a) adopts an in-progress training of the same run after a relaunch instead of
# wiping it, (b) before starting a training it raises logs/train_wanted so the pool stops
# launching evals, and waits until <= 1 eval runs and >= 100 GB is free (65 GB CLoRA + 35 GB
# eval peak fit; 3 evals + training do not), (c) does NOT own the eval pool -- qwen_watchdog.sh
# keeps exactly one gpu_pool_dyn.py alive -- and waits for the pool by name at the end.
# Idempotent: re-running skips trained runs, built arms and evaluated arms.
#
...
#
# Usage: setsid bash qwen_campaign3.sh < /dev/null > /dev/null 2>&1 &
set -u
cd "$(dirname "$0")"
...
TRAIN_FREE_MB=100000         # CLoRA claims 65 GB, an eval peaks at 35 GB
WANT=logs/train_wanted
...
train() {
  ...
  local att=0 seeds="43 43 44"
  # adopt a training of this run that is already in progress (relaunch after a crash)
  local opid; opid=$(pgrep -f -- "run_guarded.py train_cs.py .*--run_name $run( |$)" | head -1)
  if [ -n "$opid" ]; then
    say "adopting in-progress training of $run (pid $opid) -- waiting for it"
    while kill -0 "$opid" 2>/dev/null; do sleep 60; done
    if train_healthy "logs/train_${run}.log" "$MODELS/$run" \
       && "$PY" adapter_health.py --adapter "$MODELS/$run" --quarantine "$FAILED" >> "$LOG" 2>&1; then
      say "$run TRAINED OK (adopted)"
      tr '\r' '\n' < "logs/train_${run}.log" | grep -E "^\[guard\] SUMMARY" | cut -c1-400 >> "$LOG"
      return 0
    fi
    say "$run adopted attempt FAILED"; att=1; seeds="43 44"
    [ -d "$MODELS/$run" ] && { mkdir -p "$FAILED"; rm -rf "$FAILED/${run}_att0"; mv "$MODELS/$run" "$FAILED/${run}_att0"; }
    cp "logs/train_${run}.log" "logs/train_${run}_att0_failed.log"
  fi
  for seed in $seeds; do
    att=$((att+1))
    rm -rf "$MODELS/$run"
    touch "$WANT"
    # count eval PYTHON processes only (the bash -c wrapper's cmdline also contains the name)
    until [ "$(free_mb)" -ge "$TRAIN_FREE_MB" ] && [ "$(pgrep -fc "^$PY eval_one_gpu.py")" -le 1 ]; do sleep 20; done
    exec 9>"$LOCK"; flock 9
    say "training $run (attempt $att, seed $seed) -- free $(free_mb) MB, $(pgrep -fc "^$PY eval_one_gpu.py") eval(s) running"
    ...
    local tpid=$!
    sleep 240; flock -u 9; exec 9>&-; rm -f "$WANT"
    wait "$tpid"
    if train_healthy "logs/train_${run}.log" "$MODELS/$run" \
       && "$PY" adapter_health.py --adapter "$MODELS/$run" --quarantine "$FAILED" >> "$LOG" 2>&1; then
...
RUNS=(tia1_qwsw_lorawd_wd0p3_lr1e4_s43
      ...
      tia1_qwsw_clora_k1024_lr3e4_s43)
ARGS=("--method lora --lora_r 32 --lora_alpha 64 --weight_decay 0.3 --learning_rate 1e-4"
      ...
      "--method clora --lora_r 32 --lora_alpha 64 --clora_k 1024 --clora_lambda 1.0 --learning_rate 3e-4")
# 7th configuration (Guy, 2026-09-15 05:40): CLoRA at Llama's CLoRA learning rate 3e-4 --
# the magnitude-matched cell that separates "architecture" from "operating point" (pool:
# 85.84 / 37.88 / F 0.286, dw_sv_max 26 vs Llama CLoRA 3e-4's 20.7; all pool seeds healthy).
# LoRA+wd cannot serve: under wd 0.3 its Qwen update saturates at dw_sv_max ~10-13 for any LR.

say "=== Qwen campaign v3 (concurrent, restartable) start ==="
mkdir -p jobs logs
[ -f "$QUEUE" ] || : > "$QUEUE"
sed -i '/^STOP$/d' "$QUEUE"                 # a relaunch re-opens the queue
# configurations 1-2: (re-)enqueue is idempotent (pool skips evaluated arms; same-hash lines are done)
enqueue "${RUNS[1]}" first
enqueue "${RUNS[0]}" last
...
echo "STOP" >> "$QUEUE"
say "all training done, STOP queued; waiting for eval pool to drain"
sleep 120; while pgrep -f gpu_pool_dyn.py >/dev/null; do sleep 60; done
say "=== Qwen campaign v3 complete: $(grep -c 'DONE  job.*rc=0' logs/tQd_pool.log) ok, $(grep -c 'FAILED (no more' logs/tQd_pool.log) failed ==="
