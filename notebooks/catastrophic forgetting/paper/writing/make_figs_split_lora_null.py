...
HERE = "/home/guyb/UIOrthoLoRA/notebooks/catastrophic forgetting"
...
# CorDA EXCLUDED from the active set (data under re-validation; see Limitations). Its style entries
...
METHODS = ["lora", "lora_null", "lorawd", "milora", "clora", "dora", "sclora"]   # 7 assessed adapters (CorDA withheld)
PRETTY = {"lora": "LoRA", "lora_null": "LoRA-Null", "lorawd": "LoRA+wd(0.3)", "milora": "MiLoRA",
          "clora": "CLoRA", "dora": "DoRA", "corda": "CorDA", "sclora": "SC-LoRA"}
COL = {"lora": "#0072B2", "lora_null": "#333333", "lorawd": "#56B4E9", "milora": "#009E73",
       "clora": "#E69F00", "dora": "#CC79A7", "corda": "#D55E00", "sclora": "#882255"}
MARK = {"lora": "o", "lora_null": "*", "lorawd": "D", "milora": "^", "clora": "s",
...
def classify(run):
    ...
    m = re.match(r"(?:mtxm?_|lrswm?_|scl2m?_)?(lora_null|lorawd|lora|dora|milora|sclora|clora|corda)_", run)
    if m:
        method = m.group(1)
    else:
        parts = run.split("_")
    ...
    for rn, d in recs.items():
        ...
        if method == "corda":
            continue  # CorDA EXCLUDED: data invalid (wikitext-calib bug, fixed->nq_open re-running) AND
                      # calib<->eval distribution question (Fix 1). Under re-validation; see Limitations.
        rows.append(dict(
...
def watermark(fig):
    fig.text(0.006, 0.004, "Llama-2-7B  ·  LoRA-family  ·  s42 LR-sweep  ·  n=49 (7 adapters × 7 LRs; CorDA withheld)",
             ha="left", va="bottom", fontsize=7, color="0.55", style="italic")

CAVEAT = ("Caveat: single seed (s42), n=7 per adapter. CorDA WITHHELD (under re-validation: calibration fidelity, see Limitations).\n"
          "SC-LoRA below-curve deviation provisional pending eval-matched calibration + seeds 43/44.")
...
# pending a calibration-distribution sensitivity test (nq_open vs eval-matched) + seeds 43/44 (Limitations).
ON_CURVE = ["lora", "lora_null", "lorawd", "milora", "clora", "dora"]
OFF_CURVE = ["sclora"]
...
def fig0_hero():
    ...
    sensitivity; see Limitations). Annotate r/R2, base ceiling, the provisional outlier."""
    rows = SWEEP
    ...
    ax.plot(xx, fit["pred"](xx), color="#222", lw=3.0, ls="-", zorder=3,
            label=(f"$\\bf{{Magnitude\\ law}}$ (6 on-curve adapters)\n"
    ...
    for m in OFF_CURVE:
        ...
        ax.annotate(f"{PRETTY[m]}: below the law\n"
                    f"(-4.15 pp ANCOVA residual, provisional;\npending calib-distribution sensitivity)",
                    xy=(ax_x, ax_y),
    ...
    # (E2) star the LoRA+wd best-adapt operating point: weight decay walks DOWN the same curve.
    wd_rows = [r for r in rows if r["method"] == "lorawd" and r["adapt"] is not None]
    if wd_rows:
        wb = max(wd_rows, key=lambda r: r["adapt"])
        ax.scatter([wb["F"]], [wb["ret"]], s=620, marker="*", c=COL["lorawd"],
                   edgecolor="k", linewidth=1.3, zorder=9)
        ax.annotate("LoRA+wd best point (starred): CS-8 81.6 / ret 25.6\n"
                    "at the smallest update in the sweep — the wd knob\n"
                    "walks down the same curve",
                    xy=(wb["F"], wb["ret"]),
                    xytext=(0.015, 0.975), textcoords="axes fraction",
                    fontsize=9.5, color="#1f6fa8", fontweight="bold", ha="left", va="top",
                    arrowprops=dict(arrowstyle="->", color="#1f6fa8", lw=1.8,
    ...
    ax.legend(handles=on_h + off_h, loc="lower left", ncol=2, fontsize=9.2,
              title="six on-curve adapters  vs  the one provisional deviator",
              title_fontsize=9.5, framealpha=0.95)

    caveat_footer(fig, y=0.008)
    fig.subplots_adjust(left=0.085, right=0.975, top=0.93, bottom=0.16)
...
def fig1_magnitude_law():
    """(E3) Axis-choice exhibit, chosen axis prominent: one large panel for the token-weighted
    Frobenius norm; the two rejected spectral axes as half-size side panels."""
    rows = SWEEP
    fig = plt.figure(figsize=(12.5, 5.6))
    gs = fig.add_gridspec(2, 2, width_ratios=[1.75, 1], hspace=0.42, wspace=0.22)
    ...
    fits = {}
    for ax, key, xlabel, main in panels:
        x = arr(rows, key); y = arr(rows, "ret")
        good = np.isfinite(x) & np.isfinite(y) & (x > 0)
        x, y = x[good], y[good]
        fit = fit_line_logx(x, y); fits[key] = fit
        xx = np.logspace(np.log10(x.min()), np.log10(x.max()), 200)
        ax.plot(xx, fit["pred"](xx), color="0.25", lw=2.2 if main else 1.6, ls="--", zorder=2)
        for m in METHODS:
            mr = [r for r in rows if r["method"] == m]
            mx = arr(mr, key); my = arr(mr, "ret")
            g = np.isfinite(mx) & np.isfinite(my) & (mx > 0)
            ax.scatter(mx[g], my[g], s=(80 if m in SIMPLE else 50) if main else 26,
                       c=COL[m], marker=MARK[m],
                       edgecolor="k" if m in SIMPLE else "0.3",
                       linewidth=(0.9 if m in SIMPLE else 0.4) if main else 0.3,
                       alpha=0.95, zorder=5 if m in SIMPLE else 4)
        ax.set_xscale("log"); clean_logx(ax)
        ax.set_xlabel(xlabel, fontsize=11.5 if main else 9)
        badge = f"$r$={fit['r']:.2f}\n$R^2$={fit['r2']:.2f}"
        ax.text(0.035, 0.05, badge, transform=ax.transAxes,
                fontsize=12 if main else 9.5, fontweight="bold", va="bottom", ha="left",
                bbox=dict(boxstyle="round,pad=0.3", fc="#f3f6fb", ec="0.6", lw=0.9))
        if main:
            ax.axhline(BASE_CORE, ls=":", color="green", lw=1.4, zorder=1)
            x0, x1 = ax.get_xlim()
            ax.text(10**(np.log10(x1) - 0.02*(np.log10(x1)-np.log10(x0))), BASE_CORE+0.35,
                    "base ceiling (no FT)", color="green", fontsize=8.4, va="bottom", ha="right")
        else:
            ax.tick_params(labelsize=8)
    for spine in axMain.spines.values():
        spine.set_visible(True); spine.set_color("#0072B2"); spine.set_linewidth(2.2)
    axMain.set_title("CHOSEN: tightest, method-neutral fit", color="#0072B2", fontsize=12, pad=6)
    axS1.set_title("rejected: looser", color="0.45", fontsize=9.5, pad=4)
    axS2.set_title("rejected: loosest (spiky inits inflate it)", color="0.45", fontsize=9.5, pad=4)
...
def fig2_fairness_residuals():
    """(E1) Residual-only fairness exhibit: per-method residual distributions from the pooled
    spline law, green law zone, significance stars, on-plot ANCOVA annotation. The redundant
    left scatter panel is dropped (fig0 already shows the data)."""
    rows = SWEEP
    x = arr(rows, "F"); y = arr(rows, "ret")
    g = np.isfinite(x) & np.isfinite(y) & (x > 0)
    rows = [r for r, gg in zip(rows, g) if gg]
    ...
    # global smooth fit (spline on log-x) = the method-agnostic law
    order = np.argsort(lx)
    spl = UnivariateSpline(lx[order], y[order], k=3, s=len(lx)*8.0)
    resid = y - spl(lx)
    ss_tot = np.sum((y - y.mean())**2)

    # intercept ANCOVA on the linear log-x baseline (clean DoF)
    ...
    X = np.column_stack([M, lx])
    beta, *_ = np.linalg.lstsq(X, y, rcond=None)
    ss_res_full = np.sum((y - X @ beta)**2)
    ...
    bp = ax.boxplot(data, orientation="horizontal", widths=0.62, patch_artist=True, showfliers=False,
    ...
    for i, (rr, c) in enumerate(zip(data, cols)):
        ...
        ax.scatter(rr, np.full(len(rr), i+1)+jit, s=48, c=c, edgecolor="k",
                   linewidth=0.4, alpha=0.9, zorder=5)
    ax.axvline(0, color="0.25", lw=1.6, ls="--")
    ax.axvspan(-2.5, 2.5, color="#2ca02c", alpha=0.07, zorder=0)
    ax.text(0, ax.get_ylim()[1], "", fontsize=1)  # noop spacing
    ax.set_yticks(range(1, len(labels)+1)); ax.set_yticklabels(labels, fontsize=11)
    for tick, m in zip(ax.get_yticklabels(), methods_here):
        if m in OFF_CURVE:
            tick.set_fontweight("bold"); tick.set_color(COL[m])
    ax.set_xlabel("Retention residual from the pooled magnitude law  [pp]   (> 0 = above the law)",
                  fontsize=12)
    ax.set_title("Six of seven adapters straddle the law; SC-LoRA sits below it (provisional)",
                 fontsize=13.5, pad=10)
    # green law zone label (top-right corner of the zone; SC-LoRA's top-row points sit left of it)
    ax.text(2.4, len(labels) + 0.42, "the law zone\n(indistinguishable from pooled curve)",
            fontsize=8.4, color="#1d6b1d", ha="right", va="top", style="italic")
    # widen x for the mu annotations
    x0, x1 = ax.get_xlim(); ax.set_xlim(x0, x1 + 0.30*(x1-x0))
    lab_x = ax.get_xlim()[1] - 0.01*(ax.get_xlim()[1]-ax.get_xlim()[0])
    for i, (mu, pv, nn) in enumerate(tstats):
        stars = "**" if pv < 0.01 else ("*" if pv < 0.05 else "")
        ax.text(lab_x, i+1, f"μ={mu:+.1f}{stars}  (p={pv:.2f})" if pv >= 0.01 else
                f"μ={mu:+.1f}{stars}  (p={pv:.3f})",
                va="center", ha="right", fontsize=9.5, color=cols[i], fontweight="bold")
    # on-plot ANCOVA headline (E1), parked in the empty left-middle region
    ax.text(0.015, 0.62,
            f"{df1} free per-method intercepts\n"
            f"buy $\\Delta R^2$ = {r2_full - r2_lin:.2f}  ($R^2$ {r2_lin:.2f} → {r2_full:.2f});\n"
            f"$F_{{{df1},{df2}}}$ = {F_stat:.1f}, p < 0.001 —\n"
            "driven almost entirely by SC-LoRA\n"
            "(provisional; calibration confound)",
            transform=ax.transAxes, fontsize=9.5, va="top", ha="left", color="0.15",
            bbox=dict(boxstyle="round,pad=0.4", fc="#fffbe6", ec="#c9a227", lw=1.0))
    watermark(fig)
    fig.tight_layout(rect=[0, 0.06, 1, 0.99])
    caveat_footer(fig, y=0.004)
    p = os.path.join(OUT, "fig2_fairness_residuals.png")
    fig.savefig(p, bbox_inches="tight"); plt.close(fig)
    stat = dict(r2_global=1 - np.sum(resid**2)/ss_tot, r2_lin=r2_lin, r2_full=r2_full,
                F=F_stat, p=p_anova, df1=df1, df2=df2,
                per_method=dict(zip([PRETTY[m] for m in methods_here], tstats)))
    return p, stat

# ================================================================ FIG 3
def fig3_pareto():
    """(E4) Clean best-operating-points exhibit: each method's best-adapt point only, both
    domains, base-ceiling line. Full trajectories live in fig9; this is the summary plane."""
    fig, axes = plt.subplots(1, 2, figsize=(13.2, 5.4))
    for ax, (dom, rows, adapt_name) in zip(
            axes, [("cs", SWEEP, "Commonsense-8 accuracy"),
                   ("math", SWEEP_M, "GSM8K accuracy")]):
        pts = []
        for m in METHODS:
        ...
        for m, b in pts:
            star = (m == "lorawd")
            ax.scatter([b["adapt"]], [b["ret"]], s=520 if star else 170,
                       marker="*" if star else MARK[m], c=COL[m],
                       edgecolor="k", linewidth=1.2 if star else 0.9, zorder=6 if star else 5)
            dx, dy = (-1.2, -0.8) if m in ("lora",) else (0.5, 0.45)
            if m == "lorawd":
                dx, dy = (0.12, -0.62) if dom == "cs" else (0.5, 0.45)
            if dom == "math" and m == "dora":
                dx, dy = 0.6, -1.1
            ax.annotate(PRETTY[m], xy=(b["adapt"], b["ret"]),
                        xytext=(b["adapt"]+dx, b["ret"]+dy), fontsize=9,
                        color=COL[m] if m != "lora_null" else "0.2", fontweight="bold")
        ...
        if dom == "cs":
            ax.set_xlim(76.5, 83.4); ax.set_ylim(21, 27.2)
        else:
            ax.set_xlim(30, 54); ax.set_ylim(21, 27.2)
        ax.text(ax.get_xlim()[0]+0.1*(ax.get_xlim()[1]-ax.get_xlim()[0]), BASE_CORE+0.12,
                "base retention ceiling (no FT)", color="green", fontsize=8.6, va="bottom")
        ax.set_title(("Commonsense (7 adapters)" if dom == "cs" else
                      "Math / GSM8K (sparse: LoRA, LoRA+wd, DoRA)"))
    axes[0].annotate("LoRA+wd: best adaptation AND best\nretention among the strong adapters,\n"
                     "at the smallest update (0.394)",
                     xy=(81.6, 25.55), xytext=(0.975, 0.055), textcoords="axes fraction",
                     fontsize=9, color="#1f6fa8", fontweight="bold", ha="right", va="bottom",
                     arrowprops=dict(arrowstyle="->", color="#1f6fa8", lw=1.6,
                                     connectionstyle="arc3,rad=0.25"),
                     bbox=dict(boxstyle="round,pad=0.35", fc="white", ec="#56B4E9", lw=1.2))
    fig.suptitle("Best operating point per adapter (each at its own best-adapt learning rate)",
                 y=0.99, fontsize=13.5)
    watermark(fig)
    fig.tight_layout(rect=[0, 0.075, 1, 0.96])
    cfg_note_footer(fig, y=0.030)
...
CFG_NOTE = ("Note: ranks / secondary configs are NOT matched across methods "
            "(LoRA/DoRA/LoRA-Null r16, MiLoRA/SC-LoRA r32, CLoRA k1024) — cross-method gaps are partly "
...
def op_points_table():
    ...
    for d in data:
        ...
        row = [("LoRA (plain)" if d["method"] == "lora" else PRETTY[d["method"]]),
...
if __name__ == "__main__":
    ...
    print("\n=== FIG0 HERO: magnitude law fit on 6 on-curve adapters ===")
    ...
    print("\n=== MAIN LATEX TABLES: SKIPPED (paper-local variants are hand-maintained; "
          "canonical generator owns paper/table_main_*.tex) ===")
    pcs = pmath = None
