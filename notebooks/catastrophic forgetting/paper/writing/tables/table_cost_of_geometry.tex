...
\caption{\textbf{Training cost and retention at matched adaptation.} Cost
relative to LoRA{+}wd, with the retention gap at matched adaptation alongside.
Train wall-clock
...
Plain LoRA, which modifies no structure, sits $2.2$ points below the envelope as
well, so the gaps in that column are not specific to the structure-modifying
adapters.
