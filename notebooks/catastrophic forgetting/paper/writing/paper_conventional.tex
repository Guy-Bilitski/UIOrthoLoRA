...
\paragraph{Reference directions.} For each adapted matrix we decompose the
base weight $W_0$ once and cache four sets of $256$ unit vectors: its $256$
largest and $256$ smallest left singular vectors ($U_{\mathrm{top}},
U_{\mathrm{bot}}$, the output side) and the corresponding right singular
vectors ($V_{\mathrm{top}}, V_{\mathrm{bot}}$, the input side).

\paragraph{Update directions.} The update $\Delta W = (\alpha/r)BA$ has $r$
output directions $u_1,\dots,u_r$, $r$ input directions $v_1,\dots,v_r$, and
strengths $s_1 \ge \dots \ge s_r$: its singular value decomposition.
(Implementation note: the code obtains this decomposition from the saved
low-rank factors, through a thin QR decomposition of each factor and an
$r \times r$ decomposition of their core, so the dense matrix is never
formed; the result is identical to a direct decomposition.)

\paragraph{The energy shares.} For one update direction $u_i$, let $c_i$ be
the squared length of its projection onto the reference set: $c_i$ is one
when $u_i$ lies entirely inside the $256$ reference directions and zero when
it is orthogonal to all of them. The share is the average of these overlaps
weighted by squared strength, so a direction the update barely uses barely
counts:
\begin{equation*}
  e_{\mathrm{top}} = \frac{\sum_i s_i^2\, c_i}{\sum_i s_i^2}
  = \frac{\lVert U_{\mathrm{top}} U_{\mathrm{top}}^{\!\top}\, \Delta W
  \rVert_F^2}{\lVert \Delta W \rVert_F^2},
\end{equation*}
the fraction of the update's squared Frobenius norm that survives projection
onto the base model's $256$ principal output directions. Repeating this with
$U_{\mathrm{bot}}$, and with the input directions $v_i$ against
$V_{\mathrm{top}}$ and $V_{\mathrm{bot}}$, gives the other three shares.
Dividing by $\sum_i s_i^2$ makes each share scale-free, so it cannot carry
magnitude information. A raw share has no meaningful zero, since a random
direction already lands about its dimensional share in any reference set
(near $6\%$ for $256$ of $4096$ directions), which is why the figure reads
shares relative to the no-target methods below.
