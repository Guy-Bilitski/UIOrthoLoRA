...
\paragraph{Metric 3: update geometry.} Many retention-aware designs are built
around where the update sits along the base weight's spectrum, some placing it
in the major components and some in the minor ones
(Section~\ref{sec:related}); we thus measure that placement on the trained
adapters themselves. For each saved update $\Delta W = (\alpha/r)BA$ we take
the singular value decomposition of the corresponding base weight $W_0$ and
ask how much of the update falls in its $256$ strongest directions and how
much in its $256$ weakest. This gives four energy shares: two on the output
side, $e_{\mathrm{top}}$ and $e_{\mathrm{bot}}$, against the left singular
vectors of $W_0$, and two on the input side,
$e^{\mathrm{in}}_{\mathrm{top}}$ and $e^{\mathrm{in}}_{\mathrm{bot}}$,
against the right ones. Each share is the fraction of the update's energy
landing in that set of directions, and therefore records where the update
goes and never how large it is; Appendix~\ref{app:geoproj} gives the
calculation step by step. Separately we compute the update's stable rank,
$\lVert\Delta W\rVert_F^2 / \smax^2$ with $\smax$ its largest singular value,
which counts how many directions effectively carry the update and is near $1$
when a single direction dominates. Stable rank describes the update's shape
...
\caption{For each method we
measure the energy its update concentrates in four components: the major
and the minor SVD components of the base weight, on the input and on the
output side. Each energy is divided by the pooled median of the same energy
over the methods that state no target subspace (LoRA, LoRA{+}wd, DoRA) at
the same learning rate; each bar shows the most prominent of the four
ratios for that model, and its label names the component. A ratio of one means indistinguishable from an
unconstrained update, and the shaded band is the $99$th percentile of the
same statistic among the plain runs alone (metric and construction:
Appendix~\ref{app:geoproj}).}
\label{fig:geometry}
\end{figure}

\paragraph{Geometry identifies the designs.}
Figure~\ref{fig:geometry} tests whether each design's update lands where
an unconstrained update lands, on four components of the base weight's
spectrum (metric and construction in Appendix~\ref{app:geoproj}). The
concentration indeed surfaces the design:
every method that constrains update geometry separates from the plain
adapters on both base models (SC-LoRA, MiLoRA, and LoRA-Null), while the
three plain methods remain within the band produced by resampling the
plain runs. CLoRA is the exception, consistent with its constraint acting on a randomly drawn subspace that these measures do not capture. Because the measures are scale-invariant energy shares, they cannot reflect update magnitude by construction. Thus, the constraints clearly alter update geometry, but this geometric variation has limited predictive power for retention: after fitting retention on magnitude and method within each family, the remaining method differences reach about five points among the swept methods, compared with roughly $20$--$40$ points spanned by the magnitude relationship (Appendix~\ref{app:exhibits}).
...
\paragraph{The calculation.} The inputs are the base weight $W_0$ and the
update $\Delta W = (\alpha/r)BA$, two matrices of the same shape. Take the
singular value decomposition of $W_0$, keep its first $256$ left singular
vectors as the columns of $U_{\mathrm{top}}$ and its last $256$ as the columns
of $U_{\mathrm{bot}}$. Then
\begin{align*}
  e_{\mathrm{top}} &= \frac{\lVert U_{\mathrm{top}}^{\!\top}\, \Delta W
  \rVert_F^2}{\lVert \Delta W \rVert_F^2}, \\[2pt]
  e_{\mathrm{bot}} &= \frac{\lVert U_{\mathrm{bot}}^{\!\top}\, \Delta W
  \rVert_F^2}{\lVert \Delta W \rVert_F^2},
\end{align*}
where $\lVert\cdot\rVert_F^2$ squares every entry of a matrix and adds the
results. That is the entire measure: the share of the update's squared entries
that remains once only its components along the base weight's $256$ strongest,
or its $256$ weakest, output directions are kept.

\paragraph{The other two shares.} Multiplying on the right instead, by the
first or the last $256$ right
singular vectors, gives the two input-side shares
$e^{\mathrm{in}}_{\mathrm{top}}$ from $\Delta W\,V_{\mathrm{top}}$ and
$e^{\mathrm{in}}_{\mathrm{bot}}$ from $\Delta W\,V_{\mathrm{bot}}$. The
denominator is always $\lVert\Delta W\rVert_F^2$. The principal and the minor
share do not sum to one: on a matrix with $4096$ output directions the two
sets cover $512$ of them, and the middle of the spectrum is counted by
neither.

\paragraph{The same quantity, one update direction at a time.} Write the
update's own singular value decomposition as directions $u_1,\dots,u_r$ with
strengths $s_1 \ge \dots \ge s_r$, and let $c_i = \lVert
U_{\mathrm{top}}^{\!\top} u_i \rVert^2$ be the fraction of direction $u_i$
lying inside the $256$ reference directions, one when it lies entirely inside
them and zero when it is orthogonal to all of them. Then $e_{\mathrm{top}} =
\sum_i s_i^2 c_i / \sum_i s_i^2$: every direction the update pushes in
contributes its overlap with the reference set, weighted by how hard the
update pushes there, so a direction the update barely uses barely counts.

\paragraph{Scale and baseline.} Dividing by $\lVert\Delta W\rVert_F^2$ makes
each share scale-free, so multiplying the update by a constant leaves it
unchanged and the shares cannot carry magnitude information. A raw share also
has no meaningful zero, since a random direction already lands about its
dimensional share in any reference set, near $6\%$ for $256$ of $4096$
directions. The figure below therefore reads every share relative to the
methods that target nothing.
...
\paragraph{Implementation.} The decomposition of $W_0$ is computed once per
adapted matrix and cached. The update is never formed densely: the code works
from the saved factors $B$ and $A$, through a thin QR decomposition of each
and an $r \times r$ decomposition of their core, which also supplies the
singular values used for stable rank. The shares it returns are identical to
those of the direct computation above.

\paragraph{How Figure~\ref{fig:geometry} is computed.} Each method enters
at its Table~\ref{tab:grand} operating point. For each of the four shares,
the method's median is divided by the pooled median of the three no-target
methods (plain LoRA, LoRA{+}wd, DoRA) trained at the same learning rate in
the same family, keeping whichever of the ratio and its reciprocal is
larger, so placing half the reference energy counts as a two-fold difference
just as placing twice it does. Each model's bar is the largest of the four
fold-differences, labeled with the share that produced it; one means the
update is indistinguishable from a method that targets nothing. The shaded
band is the $99$th percentile of the same largest-fold statistic within
the no-target runs alone, resampling training cells over $4{,}000$ draws,
so the selection of the maximum is applied to the null as well: a bar
inside the band is indistinguishable from sampling variation among plain
runs.
...
\input{tables/table_geometry_battery.tex}
