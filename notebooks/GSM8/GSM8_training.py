...
import datasets
import numpy as np
from peft import UIOrthoLoRAConfig, RandLoraConfig, LoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel, LoraRuntimeConfig
# from peft import RandLoraConfig, LoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel, LoraRuntimeConfig
...
def build_model(script_args, checkpoint_dir):
    ...
    if not script_args.full_finetune:
        if checkpoint_dir is not None:
            ...
            #     r                = 128,             # <-- **raise for fewer params / lower for faster training**
            #     randlora_alpha   = 64,           # (≈ 16×r) – scale the update
            #     randlora_dropout = 0,          # regularization

            #     # where to inject adapters -------------------------------------------------
            #     target_modules   = ["q_proj", "k_proj", "v_proj", "o_proj",
            #                         "gate_proj", "up_proj", "down_proj"],

            #     # reproducibility & checkpoint size ----------------------------------------
            #     projection_prng_key = 42,         # seed for random bases
            # )
