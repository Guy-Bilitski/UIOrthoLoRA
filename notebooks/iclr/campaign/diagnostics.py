...
def matrix_norms(x):
    ...
    op = values[0].item() if values.numel() else 0.
    energy = x.square().sum().item()
    return dict(frobenius=math.sqrt(energy), operator=op,
                stable_rank=energy / op**2 if op else None)
...
def scaler_summary(x, near_zero=1e-12):
    ...
    return dict(mean=mean, rms=rms, centered_l2=centered,
                centered_rms_ratio=sd/rms if rms else None,
                centered_l2_over_rms=centered/rms if rms else None,
                cv=sd/abs(mean) if abs(mean) > near_zero * max(1.,rms) else None,
                cv_undefined=abs(mean) <= near_zero * max(1.,rms))
...
def subspace_report(old, new):
    ...
    cosine = torch.linalg.svdvals(old.T @ new).clamp(0.,1.)
    sine2 = (1.-cosine.square()).clamp_min(0.)
    return dict(cosines=cosine.tolist(), largest_sine=sine2.max().sqrt().item(),
                fraction_above_30_degrees=(sine2 > .25).double().mean().item(),
...
def orientation_nulls(delta, u_ref, v_ref, k, seeds):
    ...
    for left_seed,right_seed in seeds:
        ...
        rotated=(q*s)@r.T
        results.append(dict(left_seed=left_seed,right_seed=right_seed,
                            original_frame=block_summary(rotated,u_ref,v_ref,k),
                            random_frame=block_summary(delta,
                                haar_basis(m,m,left_seed,dtype=delta.dtype,device=delta.device),
                                haar_basis(n,n,right_seed,dtype=delta.dtype,device=delta.device),k),
...
def diagnose_layer(layer, cutoffs, orientation_seeds=()):
    ...
    bounds={"LL":en*s*dn+mu*h*nu,"LT":en*s*nu+mu*h*dn,
            "TL":mu*s*dn+en*h*nu,"TT":mu*s*nu+en*h*dn}
    actual={key:matrix_norms(b)["operator"] for key,b in blocks.items()}
    result["factors"]=dict(e=en,d_s=dn,s=s,h_s=h,mu=mu,nu=nu,
                            mixing_left_frobenius=torch.linalg.vector_norm(me).item(),
                            mixing_right_frobenius=torch.linalg.vector_norm(md).item(),
                            normalized_mu=mu/en if en else None,normalized_nu=nu/dn if dn else None,
                            core_frobenius=torch.linalg.vector_norm(layer.tail_core()).item())
    result["bounds"]=dict(operator_norms=actual,right_hand_sides=bounds,
    ...
    result["mean_scaler_identity_subtraction"]["interpretation"]="hypothesis-specific reference; not exact leading removal"
    result["drift"]={}
    for cutoff in sorted(set(cutoffs)|{k}):
        ...
        result["drift"][str(cutoff)]={
            ...
            "reference_sigma_k":a,"reference_sigma_k_plus_1":b,
            "effective_sigma_k":new_s[cutoff-1].item(),"effective_sigma_k_plus_1":new_s[cutoff].item(),
            "absolute_gap":gap,"relative_gap":gap/a if a else None,
    ...
    result["no_crossing_sufficient_condition"]={"applicable":applicable,"satisfied":sufficient,
                                                 "adapted_tail_operator":tail_operator,
                                                 "failure_implies_instability":False}
    m,n=w.shape
    result["dimension_only_reference"]={"LL":k*k/(m*n),"LT":k*(n-k)/(m*n),
                                         "TL":(m-k)*k/(m*n),"TT":(m-k)*(n-k)/(m*n),
...
def aggregate_layers(reports, reference_energies, delta_kind="total"):
    ...
    fractions={key:sum(r["block_energy"][key] for r in records)/total if total else None
               for key in ("LL","LT","TL","TT")}
    defined=[r for r in records if r["energy"]>0]
    means={key:sum(r["fractions"][key] for r in defined)/len(defined) if defined else None
           for key in ("LL","LT","TL","TT")}
    return dict(module_count=len(records),defined_fraction_module_count=len(defined),statistical_unit="training_seed",
                ...
                pooled_fractions=fractions,equal_module_mean_fractions=means,
