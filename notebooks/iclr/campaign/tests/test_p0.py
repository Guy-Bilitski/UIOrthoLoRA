...
from notebooks.iclr.campaign.protocol import (
    Resources, checkpoint_steps, classification_metrics, first_tranche, magnitude_match,
)
from notebooks.iclr.campaign.spectral import (
    SpectralConfig, SpectralLinear, block_summary, coordinate_blocks, haar_basis,
    mixing_squared, regularization,
)
...
@pytest.mark.parametrize("shape,rotation,scalers,leading", itertools.product(
    [(6, 6), (8, 6), (6, 8)], [0, 2], [False, True], [False, True]))
def test_forward_delta_merge_disable_reload(shape, rotation, scalers, leading):
    ...
    with torch.no_grad():
        m.h.add_(torch.tensor([.13, -.07], dtype=m.h.dtype))
        if scalers:
            m.e.add_(torch.linspace(-.03, .05, shape[0]))
            m.d.add_(torch.linspace(.08, -.06, shape[1]))
        if rotation:
            for r in (m.left_rotation, m.right_rotation):
                r.parametrizations.weight.original.add_(torch.randn_like(r.parametrizations.weight.original) * .1)
...
def test_full_rectangular_blocks_and_known_locations(shape):
    ...
    for label in ("LL", "LT", "TL", "TT", "zero"):
        ...
        if label == "zero":
            ...
            assert summary["fractions"][label] == pytest.approx(1.)
            ...
            assert scaled["fractions"][label] == pytest.approx(1.)
    with pytest.raises(ValueError, match="complete"):
        coordinate_blocks(delta, m.u_ref[:, :min(shape)], m.v_ref[:, :min(shape)], m.k) if shape[0] != shape[1] else coordinate_blocks(delta, m.u_ref[:, :-1], m.v_ref, m.k)


def test_crossing_does_not_imply_ranked_stability():
    w = torch.diag(torch.tensor([2., 1.], dtype=torch.float64))
    delta = torch.diag(torch.tensor([0., 2.], dtype=torch.float64))
...
def test_identity_rotations_shared_insertion_and_zero_init_gradients():
    ...
    ideal = layer(leading_identity=False, initial_coefficient=0.)
...
def test_factor_symmetries_and_mixing(leading):
    ...
    with torch.no_grad():
        m.e.add_(torch.linspace(-.2, .1, m.e.numel()))
        m.d.add_(torch.linspace(.1, .3, m.d.numel()))
...
def test_exact_penalties_and_module_aggregation():
    m = layer((8, 6))
    with torch.no_grad():
        m.e.add_(torch.linspace(-.3, .2, 8))
        m.d.add_(torch.linspace(.1, .5, 6))
    ...
    for condition in ("P1_LEFT", "P1_MIX", "P1_NORM", "P1_CENTER", "P1_DECAY_INIT"):
        a, terms = regularization({"one": m}, condition, .7)
        b, _ = regularization({"one": m, "two": m}, condition, .7)
    ...
    expected_center = sum(m.k * (s.numel()-m.k) / ((s.numel()-1)*(s.numel()+2)) * (s-s.mean()).square().sum() for s in (m.e,m.d))
    ...
    q = torch.tensor([[1.,0.],[1.,0.],[0.,1.],[0.,1.]], dtype=torch.float64) / 2**.5
    assert mixing_squared(torch.tensor([1.,1.,3.,3.], dtype=torch.float64),q).item() == 0


def test_randproj_is_independent_fixed_and_does_not_change_adapter():
    m = layer()
    saved = m.delta_total().detach().clone()
    ...
    with torch.no_grad():
        m.e.add_(torch.arange(6) / 10.)
    current = m.delta_total().detach().clone()
    penalty, _ = regularization({"m": m}, "P1_RANDPROJ", .5, {"m":(q,r)})
    expected = .5 * (mixing_squared(m.e,q)+mixing_squared(m.d,r))
...
def test_dense_tail_and_independent_learned_energy():
    ...
    assert learned.square().sum().item() != pytest.approx(total.square().sum().item() - initial.square().sum().item(), abs=1e-16)
...
def test_protocol_gate_metrics_and_manifest_immutability(tmp_path):
    ...
    assert magnitude_match(.94,1)["status"] == "failed_match"
    ...
    manifest = dict(experiment_id="iclr_6aa54397_v1",stage="smoke",condition="P1_UNREG",task="rte",seed=31415,source_revision="test")
