...
def tiny_models():
    ...
    inputs = {
        "input_ids": torch.tensor([[0, 4, 5, 6, 2], [0, 7, 8, 9, 2]]),
...
def test_original_frozen_probe_and_full_ft_embeddings():
    ...
    with torch.no_grad():
        ...
        model.roberta.embeddings.word_embeddings.weight.add_(0.2)
...
def test_paired_insertion_heads_fullft_and_cpu_reload():
    ...
    cfg = SpectralConfig(tail_size=4, initial_scaler=0.1, initial_coefficient=0.1)
...
def test_diagnostics_original_frame_and_bounds(leading, scalers):
    ...
    m = SpectralLinear(
        nn.Linear(6, 8, dtype=torch.float64),
    ...
    with torch.no_grad():
        m.h.add_(0.01)
        if scalers:
            m.e.add_(torch.linspace(-0.03, 0.02, 8))
            m.d.add_(torch.linspace(0.01, -0.02, 6))
    ...
    agg = aggregate_layers(
        {"a": report, "b": report}, {"a": m.w_pre.square().sum().item(), "b": m.w_pre.square().sum().item()}
    )
    ...
    assert drift["mean_squared_sine_all_k"] == pytest.approx(0.5)
...
def test_delivered_legacy_unrotated_correspondence(scalers, leading):
    ...
    legacy = LegacyLinear(
        ...
        initial_sigma=0.01,
        use_de=scalers,
        drop_major=not leading,
    )
    new = SpectralLinear(
        copy.deepcopy(base),
        SpectralConfig(
            tail_size=3, use_scalers=scalers, leading_identity=leading, initial_scaler=0.1, initial_coefficient=0.01
        ),
    )
    x = torch.randn(4, 8)
