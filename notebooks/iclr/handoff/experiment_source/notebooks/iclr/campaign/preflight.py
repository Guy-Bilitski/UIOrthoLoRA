...
def main():
    ...
    checks = [
        ([sys.executable, "scripts/build_mixing_tables.py", "--check"], HANDOFF),
    ...
    report = dict(
        ...
        source_files={
            str(p.relative_to(ROOT)): sha256(p) for p in sorted((ROOT / "notebooks/iclr/campaign").rglob("*.py"))
        },
