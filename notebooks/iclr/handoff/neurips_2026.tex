...
The preceding comparison leaves open whether reduced cross energy is specific to the pretrained-projector penalty or a byproduct of producing a smaller update. We therefore ran a prospective, registered control study on RTE and MRPC under a new common protocol: the same 48-matrix placement and practical form, sequence length 128, effective batch 32, an inner training/selection split with the official validation set held aside, fixed optimizer-step endpoints from the reference epoch budgets (5{,}670 steps for RTE; 2{,}760 for MRPC), and a separate calibration seed (31415). Three arms share identical initialization and data order: unregularized, the two-sided mixing penalty ($\lambda_E=\lambda_D=10^{-3}$), and an effective-delta Frobenius penalty $\beta\,\mathrm{mean}_\ell\,\|\Delta_\ell\|_F^2/\|W_{\mathrm{pre},\ell}\|_F^2$ whose coefficient was selected \emph{only} to match the mixing arm's pooled relative update norm at the fixed endpoint, under a predeclared $\pm5\%$ rule with bounded, norms-only dose refinement. Every tried dose, failure, and the selection record are retained (Appendix~\ref{app:matchedprotocol}).

On RTE the match succeeded almost exactly ($\beta=27.5$; pooled-norm error $0.2\%$). On MRPC no dose satisfied the tolerance: the pooled norm responds steeply and non-monotonically to $\beta$ near the target (Appendix~\ref{app:matchedprotocol}), and the nearest dose ($\beta=0.0075$, $13.5\%$ above target) is reported as a failed match, not asserted as matched.

\begin{table}[t]
\centering
\caption{Magnitude-matched control at the fixed-step endpoint, calibration seed 31415 ($n=1$ per cell). $\rho_F$ is the pooled relative Frobenius norm of the total effective delta; Cross, LL, and TT are pooled energy percentages in the pretrained SVD frame ($k=512$); the probe is masked-token cross-entropy under the frozen original MLM head on a fixed held-out corpus; Acc is inner-selection accuracy at the fixed step, scaled by 100. These runs use the common 128-token protocol and are not comparable to Table~\ref{tab:mixingmain} or Table~\ref{tab:glue_results_base_large}.}
...
\midrule
% BEGIN GENERATED FOCUSED MATCHED SUMMARY
% Generated by scripts/build_focused_tables.py from data/focused_norm/runs.csv.
RTE & Unregularized & $0.3629$ & $44.1$ & $45.1$ & $10.8$ & $9.16$ & $57.0$ \\
RTE & Two-sided mixing & $0.0534$ & $4.8$ & $46.3$ & $48.9$ & $8.06$ & $53.0$ \\
RTE & Norm-matched ($\beta=27.5$, matched) & $0.0533$ & $47.7$ & $36.7$ & $15.6$ & $8.97$ & $54.6$ \\
\midrule
MRPC & Unregularized & $0.3652$ & $43.4$ & $46.3$ & $10.3$ & $6.99$ & $67.4$ \\
MRPC & Two-sided mixing & $0.1220$ & $1.8$ & $25.2$ & $73.0$ & $6.29$ & $70.6$ \\
MRPC & Norm-matched ($\beta=0.0075$, nearest, failed match) & $0.1385$ & $43.7$ & $45.8$ & $10.4$ & $8.03$ & $67.4$ \\
% END GENERATED FOCUSED MATCHED SUMMARY
\bottomrule
\end{tabular}
\end{adjustbox}
\end{table}

Table~\ref{tab:matchedcontrol} gives the result. At matched magnitude on RTE, the Frobenius control leaves the pooled cross share at the unregularized level ($47.7\%$ versus $44.1\%$ unregularized), while the mixing penalty reduces it to $4.8\%$ and raises the tail share from $10.8\%$ to $48.9\%$. The MRPC contrast at the nearest dose is analogous ($43.7\%$ versus $1.8\%$ cross; tail $10.4\%$ versus $73.0\%$). Across the entire ten-dose RTE frontier, spanning pooled norms from $0.36$ to $0.024$, the cross share stays within $44$--$49\%$: within this study, magnitude control alone did not move allocation. This rules out the tested magnitude-only explanation for the geometric effect; it does not by itself establish universal spectral specificity, which requires the further frame and scaler controls of the registered plan. The mixing arm also shows the lowest probe cross-entropy at its norm on both tasks, though a single calibration seed is descriptive only. These calibration outcomes were used solely to fix the matched dose; seed-level confirmation uses the separate declared seeds.

% BEGIN GENERATED FOCUSED CONFIRMATION
% Generated by scripts/build_focused_tables.py from data/focused_norm/runs.csv.
% confirmation seeds incomplete; rerun --write once all 18 runs validate
...
The study of Section~\ref{sec:matchedcontrol} was executed prospectively under registered, hash-bound protocols; every run passed an independent whole-run validation (checkpoint reload with exact reproduction of task metrics, spectral diagnostics, and the frozen-head probe) before entering any table. The matching rule was declared before any control run: match the mixing arm's pooled relative Frobenius norm of the total effective delta (insertion offset included) at the fixed endpoint within $\pm5\%$, selecting the smallest absolute relative error and breaking exact ties toward the lower dose, with selection inputs restricted to validated fixed-step norms. The initial dose grid was $\beta\in\{0.01,1,100\}$ per task. When no initial dose satisfied the tolerance, at most two norms-only refinement rounds per task were registered before their outcomes were observed, each choosing new doses from the achieved-norm frontier alone (bracketing geometric means and log-linear interpolation); all tried doses and the immutable selection record are retained with the run artifacts.

On RTE the second round matched at $\beta=27.5$ with pooled-norm error $0.19\%$. One RTE dose ($\beta=31.62$) failed twice, bit-identically, in a merged-versus-unmerged forward-equivalence cost check whose float32 tolerance was calibrated for reload reproduction; the entry is excluded with its cause retained and its endpoint norm was never read, so the exclusion cannot be outcome-driven. On MRPC the pooled norm drops steeply and non-monotonically between $\beta=0.003$ and $\beta=0.01$ (Table~\ref{tab:matchedfrontier}), and no dose within the registered rounds satisfied the tolerance; the nearest dose ($\beta=0.0075$) is used and labeled a failed match. The non-monotonic response indicates that a Frobenius-matched control is ill-conditioned in this regime, which is itself relevant to interpreting magnitude-matching protocols. Weakly regularized MRPC runs under this recipe select their best validation checkpoint within the first few dozen steps and remain at the majority-class rate at the fixed endpoint; task-score comparisons on MRPC therefore carry this caveat.

\begin{table}[t]
\centering
\caption{Complete Frobenius-control dose frontier at the fixed-step endpoint, calibration seed 31415. Error is the absolute relative deviation of the pooled norm from the mixing arm's target (RTE $0.0534$, MRPC $0.1220$); Cross and TT are pooled energy percentages; the probe is masked-token cross-entropy. Every tried dose is shown; none is omitted.}
...
% Generated by scripts/build_focused_tables.py from data/focused_norm/runs.csv.
RTE & $0.01$ & $0.3646$ & $583.2$ & $44.0$ & $10.8$ & $10.00$ \\
RTE & $1$ & $0.2245$ & $320.7$ & $45.1$ & $11.9$ & $9.60$ \\
RTE & $10$ & $0.0918$ & $72.1$ & $46.5$ & $13.9$ & $8.67$ \\
RTE & $17.7828$ & $0.0678$ & $27.0$ & $47.0$ & $14.8$ & $9.14$ \\
RTE & $24$ & $0.0596$ & $11.6$ & $47.4$ & $15.4$ & $8.24$ \\
RTE & $25.7$ & $0.0553$ & $3.7$ & $47.5$ & $15.6$ & $8.66$ \\
RTE & $27.5$ & $0.0533$ & $0.2$ & $47.7$ & $15.6$ & $8.97$ \\
RTE & $29.5$ & $0.0515$ & $3.5$ & $47.6$ & $15.9$ & $9.16$ \\
RTE & $100$ & $0.0240$ & $55.0$ & $48.9$ & $18.3$ & $10.15$ \\
\midrule
MRPC & $0.001$ & $0.3500$ & $186.8$ & $44.3$ & $11.2$ & $7.08$ \\
MRPC & $0.00316228$ & $0.2708$ & $121.9$ & $43.9$ & $10.9$ & $5.71$ \\
MRPC & $0.0062$ & $0.1026$ & $15.9$ & $43.5$ & $10.2$ & $7.63$ \\
MRPC & $0.0068$ & $0.1968$ & $61.3$ & $44.0$ & $10.8$ & $7.39$ \\
MRPC & $0.0075$ & $0.1385$ & $13.5$ & $43.7$ & $10.4$ & $8.03$ \\
MRPC & $0.01$ & $0.0825$ & $32.4$ & $44.0$ & $10.5$ & $7.79$ \\
MRPC & $1$ & $0.0336$ & $72.5$ & $48.0$ & $16.0$ & $12.59$ \\
MRPC & $100$ & $0.0136$ & $88.9$ & $50.2$ & $19.6$ & $7.29$ \\
