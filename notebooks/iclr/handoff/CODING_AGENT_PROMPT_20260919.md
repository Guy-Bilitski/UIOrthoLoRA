...
current entry point and overrides older queue/status summaries. Read the local
...
band/head confirmations. Do not retrain them. The immediate evidence work is
temperature scaling on the existing practical checkpoints and held-aside
evaluation of the complete band/head block. Inspect current server artifacts
first and recover any work already done. The new raw ECE/Brier diagnostic is
...
prepare the missing inference/export work first. Then prepare CENTER and one
compact decoder pilot for discussion: Qwen2.5-1.5B-Instruct/GSM8K is a candidate,
