#!/bin/bash
# One GPU lane for the decoder subspace study. Claims entries from a shared queue under flock,
...
while true; do
  ...
  PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES="$UUID" PYTHONPATH=src \
    .venv/bin/python -m notebooks.iclr.decoder_pilot.subspace_runner train \
