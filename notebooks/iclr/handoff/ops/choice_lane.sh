#!/bin/bash
# One GPU lane for the CommonsenseQA replication (block 3). Claims entries from a shared queue under
# flock, runs each registered entry, then validates and completes it. The frozen reference is an entry
# like any other but runs through the `reference` subcommand, which trains nothing.
# Refuses a GPU carrying a foreign process (the runner re-checks this too) and stops at the first
# unexplained failure.
set -u
GPU="$1"; PROTOCOL="$2"; QUEUE="$3"
CHECKOUT=/media/eimtest/data/guyb/UIOrthoLoRA/notebooks/iclr-campaign-20260914
ROOT="$CHECKOUT/campaign_outputs_decoder_choice_v1"
RES="$CHECKOUT/notebooks/iclr/handoff/data/campaign_v1/RESOURCE_AUTHORIZATION_20260919_CHOICE.json"
...
while true; do
  ...
  if [ "$ENTRY" = "choice/reference/FROZEN" ]; then
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES="$UUID" PYTHONPATH=.:src \
      .venv/bin/python -m notebooks.iclr.decoder_pilot.choice_runner reference \
        --resources "$RES" --gpu "$GPU" --protocol "$PROTOCOL" > "$OUT" 2>&1
  else
    PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True CUDA_VISIBLE_DEVICES="$UUID" PYTHONPATH=.:src \
      .venv/bin/python -m notebooks.iclr.decoder_pilot.choice_runner train \
        --resources "$RES" --gpu "$GPU" --protocol "$PROTOCOL" --entry-id "$ENTRY" > "$OUT" 2>&1
  fi
  ...
  CUDA_VISIBLE_DEVICES= PYTHONPATH=.:src .venv/bin/python -m notebooks.iclr.decoder_pilot.choice_plan \
