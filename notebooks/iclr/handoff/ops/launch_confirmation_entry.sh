...
PF=notebooks/iclr/handoff/data/campaign_v1/preflight/20260916T013421Z_9d12a7eb/report.json
...
if [[ -f "$ledger" ]]; then
  while IFS= read -r directory; do
    ...
    if jq -e --arg e "$entry" --arg h "$hash" \
      '.confirmation_entry_id == $e and .phase_protocol_sha256 == $h' \
...
tmux -L "$SOCK" new-session -d -s "$session" -c "$CAMP" \
  "CUDA_VISIBLE_DEVICES= OMP_NUM_THREADS=2 MKL_NUM_THREADS=2 PYTHONPATH=src \
   ...
   --preflight $PF --purpose confirmation --calibration-protocol $protocol \
