...
from .layer import UIOrthoLoRALayer, Linear
...
class UIOrthoLoRAModel(BaseTuner):
    ...
    prefix: str = "uiortholora_"  # used for freezing logic
    ...
    # ------------------------------------------------------------------
    def _check_new_adapter_config(self, config: UIOrthoLoRAConfig) -> None:  # noqa: D401
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _check_target_module_exists(uiortholora_config: UIOrthoLoRAConfig, key: str) -> bool:  # noqa: D401
        return check_target_module_exists(uiortholora_config, key)
    ...
    def _create_and_replace(
        self,
        uiortholora_config: UIOrthoLoRAConfig,
        adapter_name: str,
        target: nn.Module,
        target_name: str,
        parent: nn.Module,
        ...
        **optional_kwargs,
        ...
        """Wrap one module with our UIOrthoLoRALayer if eligible."""
        ...
        if not self._check_target_module_exists(uiortholora_config, current_key):
            return
        ...
        # 5. Fresh wrap
        new_module = self._create_new_module(uiortholora_config, adapter_name, target, **optional_kwargs, **kwargs)
    ...
    def _create_new_module(
        ...
        **kwargs,
        ...
        # ──────────────────────────────────────────────────────────────────
        # choose the correct UIOrthoLoRA wrapper
        # ──────────────────────────────────────────────────────────────────
        ...
        common = dict(
            ...
            bias           = bias,
            uiortholora_alpha = uiortholora_config.uiortholora_alpha,
            uiortholora_dropout = uiortholora_config.uiortholora_dropout,
            init_uiortholora_weights = uiortholora_config.init_uiortholora_weights,
            fan_in_fan_out = uiortholora_config.fan_in_fan_out,
    ...
    def _mark_only_adapters_as_trainable(self, model: nn.Module) -> None:
        ...
        #     elif bias == "uiortholora_only":
        #         for m in model.modules():
        #             if isinstance(m, UIOrthoLoRALayer) and hasattr(m, "uiortholora_bias"):
        #                 if active_adapter in m.uiortholora_bias:
        #                     m.uiortholora_bias[active_adapter].requires_grad = True
        #     else:
    ...
    # Config helpers
    # ------------------------------------------------------------------
    def _prepare_adapter_config(self, cfg: UIOrthoLoRAConfig, model_cfg):  # noqa: D401
